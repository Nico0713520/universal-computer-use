...
- macOS with Cua 0.22.2: desktop compatibility, window discovery/capture, opaque element targeting, background semantic delivery, foreground pixel fallback, bounded verification, safe app launch and the schema-v3 correctness/performance evidence harness are implemented. Deterministic contracts are green; three fresh consecutive v0.2.3 real runs are still required before quoting current p50/p95 results or advancing beyond Developer Preview.
