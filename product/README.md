...
Mac Agent Preview (Developer Preview), version `0.2.8`. This is not a public Beta. The lightweight local MCP bridge lets a compatible multimodal Agent observe and operate the current desktop. The host Agent supplies the vision model and decision loop; product `0.2.8` and protocol `1.2.0` supply the validation, lifecycle, and execution bridge.
...
Requirements: macOS 14+ or Windows 10 1903+/11 x64, Node.js 22.21–22.x or 24.5+, an unlocked interactive desktop, and a host Agent that forwards MCP image content to its multimodal model.
...
When `setup` sees `HTTP_PROXY`, `HTTPS_PROXY`, `http_proxy`, or `https_proxy`, the direct CLI starts that same command once with Node's environment-proxy support unless `NODE_USE_ENV_PROXY=1` or `--use-env-proxy` is already active. Other commands are not restarted. The bootstrap inherits the terminal and child exit status and never prints the proxy value; TLS verification and the locked SHA-256 checks remain mandatory.

The installer has a 20-minute safety ceiling and a strict `COMPUTER_USE_INSTALL_TIMEOUT_MS=60000..3600000` override. A timed-out macOS installer receives TERM and then, after a five-second cleanup grace, KILL as one UCU-owned process group. UCU never deletes Cua's private install lock. Locked-script downloads have their own 60-second request deadline.
...
There is no one-click installer, DMG, or notarized public package. The user must grant macOS Screen Recording and Accessibility manually to the signed CuaDriver app, whose visible attribution is intentionally not hidden or rebranded.

## External host handoff

Only after a public exact commit exists, render a commit-bound prompt with the package entrypoint:

```bash
pnpm --silent host:test-prompt --host workbuddy \
  --repo https://github.com/Nico0713520/universal-computer-use \
  --commit <40-lowercase-hex-commit>
```

Test one host at a time and return only the strict privacy-safe v2 report. Codex and HanaAgent remain `not-tested`; WorkBuddy remains `experimental` until such external evidence passes. Multi-Agent concurrent control is deferred and is not part of v0.2.8.
...
- macOS with Cua 0.22.2: desktop compatibility, window discovery/capture, opaque element targeting, background semantic delivery, background window-pixel routing with explicit foreground escalation, bounded verification, and safe app launch are implemented. Product 0.2.8 configures each Adaptive Cursor session once and permits only bounded readback convergence (normally immediate, at most 400 ms) before serving MCP. Default `auto` hides it for background actions and observations, and shows it only for foreground pointer actions; `visible` and `hidden` are explicit startup modes. The harness has no fixed post-action sleep or simulated thinking delay. Foreground delivery can still change application focus. The exact visibility and latency claims remain pending the guarded idle-desktop Mac profile.
