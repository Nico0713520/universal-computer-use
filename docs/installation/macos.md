...
On macOS, doctor first verifies the local `/Applications/CuaDriver.app` with strict codesign, Gatekeeper, and the locked bundle identity. Only after that local app signature verification succeeds does it run `/Applications/CuaDriver.app/Contents/MacOS/cua-driver permissions status --json`. It accepts the permission JSON only when the response also attributes itself to the driver daemon and bundle ID `com.trycua.driver`; malformed or untrusted permission responses stay `unknown`. A signature failure remains a typed `engine_version_mismatch` with `runtime_signature_mismatch` and prevents the permission command and screenshot observation. A recognized capture/input permission failure may report the specific missing permission, but doctor never infers the calling Node process's privileges as CuaDriver privileges.
