...
Compatibility is evidence-based. The only status values are `verified`, `experimental`, `not-compatible`, and `not-tested`. The v0.2.8 Mac Agent Preview is not a public Beta, and a host is not promoted by configuration generation or a local shell test.
