...
- Doctor first verifies `/Applications/CuaDriver.app` with strict codesign, Gatekeeper, and the locked bundle identity. Only after local app signature verification succeeds does it run `permissions status --json`; the permission JSON is accepted only with driver-daemon attribution and bundle ID `com.trycua.driver`. Signature failure is a typed Runtime mismatch and stops before permission status or screenshot observation. Malformed or unattributed permission JSON remains unknown. System Settings retains visible CuaDriver attribution rather than a hidden UCU identity.
