...
- The Cua installer normally has a 20-minute safety deadline so its own 600-second dead-lock recovery can finish. `COMPUTER_USE_INSTALL_TIMEOUT_MS` may override only that phase with a decimal value from `60000` through `3600000`; invalid values stop before installer execution. A timeout terminates the UCU-owned macOS installer process group, gives its cleanup trap five seconds, and then force-kills that same group if necessary. Do not blindly delete `~/.cua-driver/packages/.install.lock.d`: a live Cua installer may own it, and lock recovery belongs to the pinned upstream installer.
- After the trusted system interactive-session check, doctor requires the fixed `/Applications/CuaDriver.app` path and verifies it with strict codesign, Gatekeeper, and the locked bundle identity before any Cua connection, session, tool, cursor operation, or daemon permission query. A missing app is typed `runtime_missing`; an existing app with a rejected identity is a typed Runtime signature mismatch. Only after local app verification succeeds does doctor run `permissions status --json`; the permission JSON is accepted only with driver-daemon attribution and bundle ID `com.trycua.driver`. Either identity failure stops before permission status, Runtime connection, or screenshot observation. A reported missing grant also stops before connecting. Malformed or unattributed permission JSON remains unknown. System Settings retains visible CuaDriver attribution rather than a hidden UCU identity.
...
- Discover an app/window and prefer `element_ref` for standard controls. This avoids guessing a button center and can act on hidden or off-Space macOS windows. Use an interior coordinate from the current exact-window PNG only for a visible, low-risk rectangular control with an unambiguous interior. Keep semantic targeting for destructive, ambiguous, obscured, minimized, or off-Space controls. Discovery bounds are desktop-logical metadata, never action coordinates.
